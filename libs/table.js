...
class Table {
    ...
    _potIsGood() {
        return this.players.map((player) => {
            return player.activeBetValue === -2 ||
                player.chipValue === 0 ||
    ...
    _endTurn() {
        ...
        this.activePlayer = getNextActivePlayerFromIndex(this.players, this.activePlayerIndex);
        ...
        if (this._potIsGood(previousActivePlayerIndex)) {
            if (this.board.length === 5) { // End of Hand
                ...
                const newDealerPlayer = getNextActivePlayerFromIndex(this.players, this.dealerPosition);
                ...
                this.dealer.setFirstPlayerToAct(this, this.players, this.dealerPosition, this.bigBlindPosition, this.board);
            }
        }

        this.players.forEach(player => {
            player.pokerHand = handEvaluator.getHandDescription(player.cards, this.board);
        })
    }
